...
const GraphViewer = () => {
  ...
  useEffect(() => {
    const fetchGraphData = async () => {
      ...
      try {
        const res = await fetch(`https://graphworks.vercel.app/api/graphs/${graphId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        setGraph(data);

        const tspRes = await fetch(`https://graphworks.vercel.app/api/graphs/${graphId}/tsp/runs`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleRunTsp = async () => {
    ...
    setRunningTsp(true);
    try {
      const res = await fetch(`https://graphworks.vercel.app/api/graphs/${graphId}/tsp?algo=${algorithm}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const handleDeleteTspRun = async (id) => {
    ...
    try {
      const res = await fetch(`https://graphworks.vercel.app/api/graphs/${graphId}/tsp/runs/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
