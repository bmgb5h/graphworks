...
const MyGraphs = () => {
  ...
  const fetchGraphs = async () => {
    ...
    try {
      const res = await fetch("graphworks-production.up.railway.app/api/graphs", {
        headers: {
      ...
      const graphDetails = await Promise.all(
        graph_ids.map(async (id) => {
          const detailRes = await fetch(`graphworks-production.up.railway.app/api/graphs/${id}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`graphworks-production.up.railway.app/api/graphs/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
